...
model = model = MLPMixer(in_channels=3, image_size=32, patch_size=4, num_classes=10,
                 dim=128, depth=8, token_dim=256, channel_dim=512).to(device) #ResNet18().to(device) #convnextv2_atto_cifar(num_classes=10).to(device) 
...
lottery_ticket_rewinding(model, train_loader, test_loader, './weights/mlp_ltr/', device, num_epochs = num_epochs, lr = lr)